 ...
 * |          Pete Warden <pete@petewarden.com> removed PEAR dependency   |
 * +----------------------------------------------------------------------+
 *
 * @category Net
 * @package  Net_GeoIP
...
// Pete- I've replaced all exceptions with die() so you don't have to install
// PEAR to use this class
...
class Net_GeoIP
{
    ...
    */
        ...
        if ($this->flags & self::SHARED_MEMORY) {
            ...
            if ($this->shmid === false) {
                ...
                if ($this->shmid === false) { // should never be false as loadSharedMemory() will throw Exc if cannot create
                    die("Unable to open shared memory at key: " . dechex(self::SHM_KEY));
                }
            }
        } else {
    ...
    protected function lookupCountryId($addr)
    {
        $ipnum = ip2long($addr);
        if ($ipnum === false) {
            die("Invalid IP address: " . var_export($addr, true));
        }
        if ($this->databaseType !== self::COUNTRY_EDITION) {
            die("Invalid database type; lookupCountry*() methods expect Country database.");
        }
    ...
    protected function seekCountry($ipnum)
    {
        ...
        die("Error traversing database - perhaps it is corrupt?");
    }
    ...
    public function lookupOrg($addr)
    {
        ...
        if ($this->databaseType !== self::ORG_EDITION) {
            die("Invalid database type; lookupOrg() method expects Org/ISP database.");
        }
    ...
    public function lookupRegion($addr)
    {
        ...
        if ($this->databaseType !== self::REGION_EDITION_REV0 && $this->databaseType !== self::REGION_EDITION_REV1) {
            die("Invalid database type; lookupRegion() method expects Region database.");
        }
    ...
    public function lookupLocation($addr)
    {
        ...
        if ($this->databaseType !== self::CITY_EDITION_REV0 && $this->databaseType !== self::CITY_EDITION_REV1) {
            die("Invalid database type; lookupLocation() method expects City database.");
        }
